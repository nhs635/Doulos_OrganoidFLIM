...
#ifndef FLIM_CH_START_6
#error("FLIM_CH_START_6 is not defined for FLIM processing.");
#endif
...
struct FLIM_PARAMS
{
    ...
    int ch_start_ind[6] = { 0, };
    float delay_offset[4] = { 0.0f, };
};
...
public:
    ...
    {
        // 0. Initialize
        int _nx = pParams.ch_start_ind[5] - pParams.ch_start_ind[0];
        if ((nx != _nx) || !initiated)
		...
		//// Parallel-for loop
		//memcpy(ext_src.raw_ptr(), crop_src.raw_ptr(), sizeof(float) * crop_src.length());
		//memcpy(filt_src.raw_ptr(), crop_src.raw_ptr(), sizeof(float) * crop_src.length());
  ...
  //      });
    ...
    void initialize(const FLIM_PARAMS& pParams, int _nx, int _upSampleFactor, int _alines)
    {
        ...
        ippsMin_32s(diff_ind, 5, &pulse_roi_length);
		char msg[256];
...
public:
    ...
    void operator() (const RESIZE& resize) // scans x 256 ==> 256 x 5
    {
        if (_ny != resize.ny)
        {
            intensity = FloatArray2(resize.ny, 5);
            _ny = resize.ny;
        }
        ...
        for (int i = 0; i < 5; i++)
            ippsDiv_32f_I(&intensity(0, 0), &intensity(0, 4 - i), intensity.size(0));
    }
...
public:
    ...
    void operator() (const RESIZE& resize, const FLIM_PARAMS& pParams, FloatArray2& intensity)
    {
        if (_ny != resize.ny)
        {
            mean_delay = FloatArray2(resize.ny, 5);
            lifetime = FloatArray2(resize.ny, 4);
            _ny = resize.ny;
        }
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, (size_t)resize.ny),
            ...
            for (size_t i = r.begin(); i != r.end(); ++i)
            {
				for (int j = 0; j < 5; j++)
				{
					...
					// 1. Get IRF width
					const float* pulse = &resize.ext_src(offset, (int)i);
					...
					MeanDelay_32f(resize, &resize.ext_src(offset, (int)i), offset, (int)i, maxIdx, roiWidth[j], left, md_temp);
