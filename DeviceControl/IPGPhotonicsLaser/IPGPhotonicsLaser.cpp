...
bool IPGPhotonicsLaser::EnableEmissionD(bool enabled)
{
    if (enabled)
    {
        // Set the AuxOFF input to HIGH
        m_dwCurDigValue |= 0x0800;
		if (!WriteDigitalOutput(EM_ENABLE_LINE, m_dwCurDigValue))
			return false;
    ...
    else
    {
		...
		if (!WriteDigitalOutput(EM_ENABLE_LINE, 0))
			return false;
...
bool IPGPhotonicsLaser::SetPowerLevel(uint32_t power_level)
{
    m_dwCurDigValue |= (power_level & 0x00FF);
    if (!WriteDigitalOutput(POWER_LEVEL_LINE, m_dwCurDigValue))
		return false;

    m_dwCurDigValue |= 0x0100;
    if (!WriteDigitalOutput(POWER_LATCH_LINE, m_dwCurDigValue))
		return false;
...
bool IPGPhotonicsLaser::MonitorLaserStatus()
{
    uint32_t monitor_value = 0;
	if (!ReadDigitalInput(MONITORING_LINE, monitor_value))
		return false;
...
bool IPGPhotonicsLaser::ReadDigitalInput(const char* line_name, uint32_t& value)
{
...
bool IPGPhotonicsLaser::WriteDigitalOutput(const char* line_name, uint32_t value)
{
