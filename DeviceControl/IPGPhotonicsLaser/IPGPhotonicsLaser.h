...
public:
    ...
    bool EnableEmissionD(bool enabled);
	bool SetPowerLevel(uint32_t power_level);
	bool MonitorLaserStatus();

    bool ReadDigitalInput(const char* line_name, uint32_t& value);
	bool WriteDigitalOutput(const char* line_name, uint32_t value);
